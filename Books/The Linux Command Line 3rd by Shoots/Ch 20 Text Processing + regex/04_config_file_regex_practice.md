...
---
# Day 4: Regex Text Processing with Config Files
...
A config file is like a rule sheet.

Some rules are active. Some are comments.

Regex helps ask:

```text
Which lines are comments?
Which lines are active settings?
Which setting controls SSH passwords?
Which Nginx directives are inside the file?
```

Discipline:

```text
Configuration-file regex must be stricter than casual searching.
A loose match can mix active and commented settings.
```

---

## Exercise 1: Comments versus active settings

Inspect:

```bash
cat config/sshd_config_sample
```

Find comments:

```bash
grep '^#' config/sshd_config_sample
```

Find non-comments:

```bash
grep -v '^#' config/sshd_config_sample
```

But notice possible blank lines. Better:
...
```text
grep -v '^#' removes comment lines, but blank lines are also non-comment lines.
grep '[[:alpha:]]' keeps lines containing letters.
```

---

## Exercise 2: Active versus commented PasswordAuthentication
...
Stricter active-setting command:

```bash
grep '^PasswordAuthentication' config/sshd_config_sample
```

Both commented and active forms:
...
```text
Which command should you use if you only want the active setting?
```
...
## Exercise 3: Nginx directives with indentation

Inspect:

```bash
cat config/nginx_site_sample.conf
```

Find `listen`:

```bash
grep 'listen' config/nginx_site_sample.conf
```

Find `listen` as an indented directive:
...
```text
^[[:space:]]* allows the directive to be indented.
listen must appear after the optional spaces.
```

---

## Exercise 4: Extract directive names

Question:

```text
Which directive names appear in the Nginx sample?
```

Build:

```bash
grep -E '^[[:space:]]*[a-z_]+ ' config/nginx_site_sample.conf
```

Then normalize spaces:

```bash
grep -E '^[[:space:]]*[a-z_]+ ' config/nginx_site_sample.conf | sed 's/^[[:space:]]*//'
```

Then extract first field:

```bash
grep -E '^[[:space:]]*[a-z_]+ ' config/nginx_site_sample.conf | sed 's/^[[:space:]]*//' | cut -d ' ' -f1
```
...
## Exercise 5: False positive discipline

Compare:

```bash
grep 'root' config/nginx_site_sample.conf
grep -E '^[[:space:]]*root[[:space:]]' config/nginx_site_sample.conf
```

Question:

```text
Which is safer for finding the root directive, and why?
```

Expected idea:

```text
The second is safer because it requires root to appear as a directive after optional indentation.
```
...
1. Why can `grep PasswordAuthentication` be misleading?
2. What does `^#` match?
3. What does `#?` mean with `grep -E`?
4. Why use `^[[:space:]]*` for config files?
5. Why inspect active settings separately from comments?
