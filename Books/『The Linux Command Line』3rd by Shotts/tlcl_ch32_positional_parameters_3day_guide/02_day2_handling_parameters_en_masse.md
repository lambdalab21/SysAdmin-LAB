...
```text
Scripts often receive more than one argument.
The script must handle all of them without destroying spaces, splitting names incorrectly, or guessing.
```
...
Imagine several envelopes are handed to the script:
...
A sloppy script dumps all envelopes into one pile.

A disciplined script opens each envelope separately.
