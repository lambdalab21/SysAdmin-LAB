...
---
# Day 3: Regex with `sed` Transformations
...
`sed` is an editor working on a conveyor belt.

Each line passes by. `sed` can:
...
```text
First print transformed output.
Do not edit original files until the transformation is proven.
```

---

## Exercise 1: Print selected lines with `sed -n`

Question:

```text
Show only serious log lines using sed.
```
...
Now use extended regex? Basic `sed` may not use `|` without options depending on system. Use two commands first:

```bash
sed -n '/^ERROR/p; /^WARN/p' logs/app.log
```
...
/pattern/p prints matching lines.
```

---

## Exercise 2: Delete DEBUG and lowercase error lines from output

Question:

```text
Can I show the log without DEBUG and lowercase error lines?
```

Run:

```bash
sed '/^DEBUG/d; /^error/d' logs/app.log
```

Discipline question:

```text
Did this modify logs/app.log?
```

Expected answer:

```text
No. It printed changed output only.
```

Verify:

```bash
grep -E '^(DEBUG|error)' logs/app.log
```

---

## Exercise 3: Replace hostnames in output

Question:

```text
Can I replace app01 with web01 in output only?
```

Run:

```bash
sed 's/app01/web01/g' logs/app.log
```

Verify original is unchanged:

```bash
grep 'app01' logs/app.log
```

Explain:

```text
s/app01/web01/g replaces all app01 occurrences on each line in the output.
```

---

## Exercise 4: Normalize log levels

Question:

```text
Can I turn lowercase error into uppercase ERROR in output?
```

Run:

```bash
sed 's/^error/ERROR/' logs/app.log
```

Discipline question:

```text
Why use ^error instead of error?
```

Expected idea:

```text
^error only changes error at the beginning of the line, where log levels appear.
```

---

## Exercise 5: Extract values using sed substitution

Question:

```text
Can I extract failed-login usernames using sed?
```
...
```bash
grep 'failed-login' logs/app.log | sed 's/.*user=\([^ ]*\).*//'
```

Explain carefully:

```text
.* before user= skips earlier text.
\([^ ]*\) captures non-space characters after user=.
...
```text
This is powerful, but harder to read than cut.
Prefer the simpler tool when it is clear.
```
...
1. What does `sed -n '/pattern/p'` do?
2. What does `/pattern/d` do?
3. Why is `^` useful in substitutions?
4. Does ordinary `sed` output change the original file?
5. When might `cut` be clearer than `sed`?
